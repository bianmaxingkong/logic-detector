    ...
    def _load_fallacy_patterns(self) -> Dict[FallacyType, List[Tuple[str, str]]]:
        ...
        return {
            FallacyType.AFFIRMING_CONSEQUENT: [
                ...
                # 如果...会... 变体（不用"就"而用"会"）- LogiQA 常用
                (r"如果.*会.*[。，]((?!不|没有|没|未曾|非|是|有|这个|那个|这些|那些|x|P|A|B)[^。，]*)[。，]所以.*", "肯定后件：会...所以"),
                (r"如果.*会.*[。，]((?!不|没有|没|未曾|非|是|有|这个|那个|这些|那些|x|P|A|B)[^。，]*)[。，]因此.*", "肯定后件：会...因此"),
                (r"P→Q, Q, ∴P", "肯定后件：形式逻辑"),
                # 形式逻辑中文版（含空格和中文结论词）
                (r"P→Q[,，]+Q[,，]+(因此|所以) P", "肯定后件：形式逻辑中文"),
                # LogicInference 精确匹配（带回溯引用的规则可区分modus ponens和肯定后件）
                (r"如果 (.) 则 (.)。\2 为真。因此 \1 为真", "肯定后件：则-回溯"),
                (r"若 (.) 则 (.)。\2 为真。因此 \1 为真", "肯定后件：若则-回溯"),
                (r"如果下雨.*(会|就)湿。.*湿了。*因此下雨", "肯定后件：下雨地湿"),
                # 通用肯定后件：只有X才Y。Y成功/完成了/实现了。所以X。
                (r"只有(.)才(.).*(\2(成功|发生|成立|出现|实现|完成|有了|通过|到|了|过))[。，]所以.*\1", "肯定后件：只有才"),
            ...
            FallacyType.DENYING_ANTECEDENT: [
                ...
                (r"P→Q, ¬P, ∴¬Q", "否定前件：形式逻辑"),
                # 形式逻辑中文版（含空格和中文结论词）
                (r"P→Q[,，]+¬P[,，]+(因此|所以) ¬Q", "否定前件：形式逻辑中文"),
                # LogicInference 精确匹配（用回溯引用区分modus tollens）
                # P→Q, 非P。因此非Q → 谬误
                (r"如果 (.) 则 (.)。\1 为假。因此 \2 为假", "否定前件：则-回溯"),
                (r"若 (.) 则 (.)。非 \1。因此非 \2", "否定前件：若则-回溯"),
                # 如果下雨，没下雨。因此不会湿。→ 谬误
                (r"如果下雨.*湿。没下雨。*因此.*不会湿", "否定前件：下雨-没下"),
                # 通用否定前件：如果X会Y。没/不X。所以不/没Y。
                (r"如果.*[会就].*[。，][没不不非].*[。，](所以|因此)[没不不非].*", "否定前件：中文通用"),
            ...
            FallacyType.AD_HOMINEM: [
                # 人身攻击
                (r"他.*(人品|品行|道德|素质)不好。所以他的观点.*错", "人身攻击：品德攻击"),
            ...
            FallacyType.EQUIVOCATION: [
                # 偷换概念 / 一词多义 (10 条)
                (r"(本质|其实|说白了|就是|所谓).*[。，].*不能[。，].*[。，]所以.*", "偷换概念：概念偷换"),
                (r"所谓的.*其实是.*[，。].*所以.*", "偷换概念：重新定义"),
                (r"的实质.*是.*[，。].*所以.*", "偷换概念：实质替换"),
                (r"从某种意义上.*是.*[，。].*所以.*", "偷换概念：某种意义上"),
                (r"换句话.*说.*[，。].*所以.*", "偷换概念：换句话"),
                (r"A[和与跟]B不是一回事.*[，。].*却把A.*B.*", "偷换概念：不是一回事"),
                (r"偷换概念", "偷换概念：直接提及"),
                (r"把.*和.*混为一谈[，。].*所以.*", "偷换概念：混为一谈"),
                (r"不能.*等同.*[，。].*所以.*", "偷换概念：等同错误"),
                (r"的实质就是.*[，。].*所以.*", "偷换概念：实质就是"),
                (r"本质上.*[，。].*所以.*等同于.*", "偷换概念：本质=等同"),
            ],
            FallacyType.APPEAL_TO_NATURE: [
                # 诉诸自然 (8 条)
                (r"(天然|自然)[^。，]*[，。].*所以.*更好", "诉诸自然：天然更好"),
                (r"纯天然.*[，。].*所以.*", "诉诸自然：纯天然"),
                (r"自然的?才是最好的", "诉诸自然：自然最好"),
                (r"(化学|人工|合成|添加剂).*不好[，。].*所以.*天然.*好", "诉诸自然：化学不好"),
                (r"零添加.*[，。].*所以.*", "诉诸自然：零添加"),
                (r"纯手工.*[，。].*比.*好", "诉诸自然：手工好"),
                (r"没有添加剂.*[，。].*所以.*安全", "诉诸自然：无添加安全"),
                (r"天然.*优于.*", "诉诸自然：天然优于"),
            ],
            FallacyType.ARGUMENT_FROM_IGNORANCE: [
                # 诉诸无知 (8 条)
                (r"没有证据证明.*[，。].*所以.*", "诉诸无知：没有证据"),
                (r"无法证明.*不对.*[，。].*所以.*", "诉诸无知：无法证明不对"),
                (r"不能证明不存在.*[，。].*所以.*存在", "诉诸无知：不能证明不存在"),
                (r"(科学家|专家|权威).*(说不清|不清楚|不知道)[，。].*所以.*", "诉诸无知：权威说不清"),
                (r"没人(能证明|能解释|知道).*[，。].*所以.*", "诉诸无知：没人知道"),
                (r"没有被发现.*[，。].*所以.*不存在", "诉诸无知：没发现就不存在"),
                (r"无法.*证伪.*[，。].*所以.*为真", "诉诸无知：无法证伪即真"),
                (r"不能排除.*[，。].*所以.*可能", "诉诸无知：不能排除可能"),
            ],
        }
