...
def generate_model_variation(base_text: str, model_name: str) -> str:
    """生成模型特定的变体"""
    prefixes = {
    ...
    return prefixes.get(model_name, "") + base_text
...
def generate_test_set(model_name: str, output_path: str):
    ...
    for i, base_case in enumerate(BASE_TEST_CASES):
        test_case = {
            ...
            "text": generate_model_variation(base_case["text"], model_name),
            "label": base_case["label"],
            "category": base_case["category"],
            "source_model": model_name,
            "original_text": base_case["text"],
        }
...
def main():
    ...
    for model in MODELS:
        filename = f"test_set_{model.lower().replace(' ', '_').replace('.', '').replace('-', '')}.json"
