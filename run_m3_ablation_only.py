#!/usr/bin/env python3
"""
快速运行：只测 Without M3 这一个消融配置
M3/M4数据我们还要测，但先拿到M3的数据用于论文
"""
import json
import os
import sys
...
def test_single(config_name, **kwargs):
    print(f"\n{'='*60}", flush=True)
...
if __name__ == "__main__":
    ...
    # 测试单个：Without M3
    result = test_single("Without M3 (no ConsistencyVerifier)", disable_m3=True)
    
    # 保存结果
    output_path = "/home/baibai/.openclaw/workspace/logic-detector/ablation_without_m3_result.json"
